...
cat $metadata
declare class && set_from_metadata class 'metadata.facility.plan_slug' <"$metadata" "t1.small.x86"
declare facility && set_from_metadata facility 'metadata.facility.facility_code' <"$metadata" "onprem"
declare os && set_from_metadata os 'operating_system_version.os_slug' <"$metadata"
declare pwhash && set_from_metadata pwhash 'crypted_root_password' <"$metadata"
declare state && set_from_metadata state 'metadata.state' <"$metadata" "provisioning"
